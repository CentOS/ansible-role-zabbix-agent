...
ro_mounted_devices=$(awk '$4 ~ "^ro[,$]" && $3 !~ "(squashfs|iso9660|tmpfs|nfs|ramfs)" {print $0}' /proc/mounts | wc -l)
...
awk '$4 ~ "^ro[,$]" && $3 !~ "(squashfs|iso9660|tmpfs|nfs|ramfs)" {print $0}    ' /proc/mounts >> $logfile
